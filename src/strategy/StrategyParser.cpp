...
bool StrategyConfigParser::LoadConfigFromFile(const std::string &file_path, StrategyConfig &conf) {
...
void StrategyConfigParser::ParseJsonConfig(const nlohmann::json &jsonData, StrategyConfig &config) {
...
bool StrategyConfigParser::CheckValid(const std::string &jsonString) {
    try {
        nlohmann::json jsonData = nlohmann::json::parse(jsonString);
...
bool StrategyConfigParser::getTriggerType(const std::string& filepath, std::vector<std::string>& trigger_vec) { 
    // 读取storage配置文件
...
bool StrategyConfigParser::CheckMessage(const json &j, std::vector<std::string>& trigger_vec, int8_t& bag_duration)
{
