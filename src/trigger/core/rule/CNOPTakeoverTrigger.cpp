...
bool CNOPTakeoverTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, CNOPTakeoverTrigger, "CheckCondition failed", false);
...
void CNOPTakeoverTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
void CNOPTakeoverTrigger::Updateplanning(const senseAD::msg::planning::MCUStateMachineInfo::Reader& report) {
    cnoptakeover_ = ((report.getMcuStateMachene() == senseAD::msg::planning::MCUStateMachineInfo::StateMachine::NOP_ACITVE) && 
    report.getMcuDrvOverride() == senseAD::msg::planning::MCUStateMachineInfo::Override::BOTH_OVERRIDE);
}
bool CNOPTakeoverTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[CNOPTakeoverTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
    ...
    {
        vars["cnoptakeover"] = cnoptakeover_;
    }
...
void CNOPTakeoverTrigger::NotifyTriggerContext(const TriggerContext& context) {
    if (factoryPtr_) {
