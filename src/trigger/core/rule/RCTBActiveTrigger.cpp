...
bool RCTBActiveTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, RCTBActiveTrigger, "CheckCondition failed", false);
...
void RCTBActiveTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
void RCTBActiveTrigger::UpdateASStateMachine(const senseAD::msg::tap::AsCmdLgSafe::Reader& report) {
    rctb_active_ = (report.getShadowMode() == 33); 
}

bool RCTBActiveTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[RCTBActiveTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
