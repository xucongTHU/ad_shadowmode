...
bool FCWActiveTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, FCWActiveTrigger, "CheckCondition failed", false);
...
void FCWActiveTrigger::OnMessageReceived(const std::string& topic, const TRawMessagePtr& msg) {
...
void FCWActiveTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
void FCWActiveTrigger::UpdateASStateMachine(const senseAD::msg::tap::AsCmdLgSafe::Reader& report) {
    fcw_active = (report.getShadowMode() == 9);
}

bool FCWActiveTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[FCWActiveTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
...
void FCWActiveTrigger::NotifyTriggerContext(const TriggerContext& context) {
    if (factoryPtr_) {
