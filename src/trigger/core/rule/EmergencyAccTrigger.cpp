...
bool EmergencyAccTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, EmergencyAccTrigger, "CheckCondition failed", false);
...
void EmergencyAccTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
void EmergencyAccTrigger::UpdateVehicleReport(const senseAD::msg::vehicle::VehicleReport::Reader& report) {
    // autoABSEnable_ = report.getBrake().getAbsActive();
    acceleration_ = report.getAcc().getValueActual();
    LOG_ERROR("acc: %f", acceleration_);
}


bool EmergencyAccTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[EmergencyAccTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
    ...
    std::unordered_map<std::string, TriggerConditionChecker::Value> vars;
    {
        ...
        vars["lon_acceleration"] = acceleration_;
    }
