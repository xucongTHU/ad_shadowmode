...
#include "CurveDynamicDetectionTrigger.h"
...
bool CurveDynamicDetectionTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, CNOPTrigger, "CheckCondition failed", false);
...
void CurveDynamicDetectionTrigger::OnMessageReceived(const std::string& topic, const TRawMessagePtr& msg) {
    if (topic == "/caic_pub_test/curve") {
        UpdateVehicleInfo(msg);
    } else if (topic == "/caic_pub_test/EmergencyEvasionTrigger"){
        UpdateVehicleInfo(msg);
    }

}
void CurveDynamicDetectionTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
void CurveDynamicDetectionTrigger::UpdateVehicleReport(const senseAD::msg::vehicle::VehicleReport::Reader& report) {
    curve_acceleraton_y_ = report.getImu().getAccelY();

}

void CurveDynamicDetectionTrigger::UpdateChassisReport(const gac::rscl::ChassisReportMsg::Reader& report) {
    curve_yaw_rate_ = report.getEgoMotion().getYawRate().getValue();
}

bool CurveDynamicDetectionTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[CurveDynamicDetectionTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
    ...
    {
        vars["lat_acceleration"] = curve_acceleraton_y_; 
        vars["yaw_rate"] = curve_yaw_rate_; 
    }
    std::cout << "lat_acceleration" << curve_acceleraton_y_ << std::endl;
    std::cout << "yaw_rate" << curve_yaw_rate_ << std::endl;
    bool ok = trigger_checker_.check(vars);
    
    return ok;
}

void CurveDynamicDetectionTrigger::NotifyTriggerContext(const TriggerContext& context) {
    if (factoryPtr_) {
