...
bool CNOPQuitTrigger::Proc() {
    bool ok = CheckCondition();
    CHECK_AND_RETURN(ok, CNOPQuitTrigger, "CheckCondition failed", false);
...
void CNOPQuitTrigger::UpdateVehicleInfo(const std::shared_ptr<ReceivedMsg<senseAD::rscl::comm::RawMessage>>& msg) {
...
bool CNOPQuitTrigger::CheckCondition() {
    if (!trigger_checker_.parse(trigger_condition_)) {
        LOG_ERROR("[CNOPQuitTrigger]: %s", trigger_checker_.last_error().c_str());
        return false;
    }
