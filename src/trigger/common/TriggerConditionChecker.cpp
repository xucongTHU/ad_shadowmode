...
#include "TriggerConditionChecker.h"
...
TriggerConditionChecker::~TriggerConditionChecker() = default;

bool TriggerConditionChecker::parse(const string& condition) {
    last_error_.clear();
    elements_.clear();
...
bool TriggerConditionChecker::check(const unordered_map<string, Value>& variables) {
    last_error_.clear();
...
string TriggerConditionChecker::last_error() const {
    return last_error_;
}

vector<TriggerConditionChecker::ConditionElement>
TriggerConditionChecker::get_elements() const {
    return elements_;
}
...
vector<pair<string, string>> TriggerConditionChecker::split_logical_ops(const string& expr) {
...
string TriggerConditionChecker::trim(const string& s) {
