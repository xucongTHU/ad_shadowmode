...
static const char* log_sw_version = "log version no is v25.9.0";
...
bool Logger::Init(const uint8_t toConsoleFile, int nLevel, const char* const pLogPath, const char* const csvLogPath) {
  ...
  if (m_initFlag) {
    printf("Logger has already inited\n");    // PRQA S 6001 # No logging system, only printf can be used
    return true;
  }
  ...
  if (toConsoleFile & LOG_TO_FILE) {
    ...
    if (!m_fp) {
      printf("Error: fail to open file %s\n", pLogPath);    // PRQA S 6001 # No logging system, only printf can be used
      return false;
    }
  }
...
void Logger::Uninit() {
  ...
  if (pLog) {
    delete pLog;    // PRQA S 5217 # new operator is in instance()
...
static inline void log_time(char* const buf) {
  ...
  snprintf(buf, LOG_TIME_BUF_SIZE - 1, "%04d%02d%02d %02d:%02d:%02d.%03ld",
           local.tm_year + 1900, local.tm_mon + 1, local.tm_mday, local.tm_hour,    // PRQA S  4400 # 1900 is the beginning time
...
static uint8_t Level2ColorIndex(const long& level)
{
  switch (level) {
    ...
    case LOG_LEVEL_WARNING:
      return 1;    // PRQA S 4400 # warning index
    case LOG_LEVEL_INFO:
      return 2;    // PRQA S 4400 # info index
    ...
    case LOG_LEVEL_DEBUG9:
      return 3;    // PRQA S 4400 # debug index
    default:
      return 2;    // PRQA S 4400 # info index
  }
}
...
// PRQA S 6006 ++ # mainly if conditions
...
#else
    printf("%s\n", (strColorStart + strDebugInfo + strColorEnd).c_str());    // PRQA S 6001 # thread safe
#endif
      ...
      printf("%s\r\n", strDebugInfo.c_str());
    #else
      printf("%s\n", strDebugInfo.c_str());    // PRQA S 6001 # thread safe
    #endif
