...
export AD_DATA=$(realpath ${AD_HOME}/../ad_data/ 2>&1)
export AD_LOG=$(realpath ${AD_HOME}/../ad_log/ 2>&1)
export LOG_path=${AD_LOG}
export LD_LIBRARY_PATH=${AD_HOME}/lib:$LD_LIBRARY_PATH
export SENSEAUTO_RSCL_CONF_DIR=${AD_HOME}/etc/senseauto/
export launcher_senseauto_config_dir=${AD_HOME}/etc/senseauto/
...
CASE_DIR="${BENCHMARK_DIR}/resource/cases/had"
...
simple_benchmark() {
    ...
    if [ -z "$specify_case" ] || ( [[ -n "$specify_case" && -n "$case_file" ]] && [[ "$case_file" =~ $specify_case ]] ); then
        ...
        do
            mv ${AD_LOG}/*.$pid.txt ${STAT_CASE_DIR}
...
cat << EOF
benchmark_had.sh -- Script for benchmark rscl transportation
...
Usage:
    ...
    [-c|--case]         Specify running case(had/basic/1_200hz_20Bytes.yaml)
    [-t|--tuner]        Run autotuner for range hz/msg_size test, 1 enable(default), 0 disable

Example:
    bash benchmark_x86.sh -c had/basic/1_200hz_20Bytes.yaml -s -t 0
This command will run benchmark using had/basic/1_200hz_20Bytes.yaml, with stress tool, no perf, no tuner range
EOF
}
