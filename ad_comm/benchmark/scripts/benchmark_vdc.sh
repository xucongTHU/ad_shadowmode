...
export AD_HOME=$(realpath $(
  cd $(dirname $0)
  pwd
)/../../)
if [ -f ${AD_HOME}/apps/ad_rscl_tools/env.sh ]; then
  # case 1. currently path is in ad_rscl_tools
  source ${AD_HOME}/apps/ad_rscl_tools/env.sh
else
  # case 2. ad_rscl_tools is in child directory
  env_sh=$(find -path "*/ad_rscl_tools/env.sh")
  folder=$(dirname ${env_sh})
  cd $folder
  source ./env.sh
fi
export launcher_senseauto_installed_root=${AD_HOME}
unset LOG_tostderr

# Init Constant
PID_FILE="/tmp/benchmark_launcher.pid"
RSCL_DIR="${AD_HOME}/apps/ad_rscl_tools"
BENCHMARK_DIR="${RSCL_DIR}/benchmark"
EXEC_BIN_DIR="${RSCL_DIR}/tools"
LAUNCHER_EXE_BIN="${EXEC_BIN_DIR}/benchmark_launcher"
TUNER_BIN="${EXEC_BIN_DIR}/autotuner"
CASE_DIR="${BENCHMARK_DIR}/resource/cases/vdc"
...
cat << EOF
benchmark_vdc.sh -- Script for benchmark rscl transportation

Usage:
    [-h|--help]         Show the help message
    [-p|--perf]         Start with perf
    [-s|--stress]       Add additional cpu&&io stress running case
    [-l|--list]         List available cases
    [-c|--case]         Specify running case(vdc/basic/1_200hz_20Bytes.yaml)
    [-t|--tuner]        Run autotuner for range hz/msg_size test, 1 enable(default), 0 disable

Example:
    bash benchmark_vdc.sh -c vdc/basic/1_200hz_20Bytes.yaml -s -t 0
This command will run benchmark using vdc/basic/1_200hz_20Bytes.yaml, with stress tool, no perf, no tuner range
EOF
}
...
# for some config stash in
function PushConfig() {
    cp ${launcher_senseauto_config_dir}/resource_cfg/topic_config.yaml ${launcher_senseauto_config_dir}/resource_cfg/topic_config.yaml.back
    cp ${BENCHMARK_DIR}/resource/running_config/vdc/topic_config.yaml ${launcher_senseauto_config_dir}/resource_cfg/
}

# for some config stash out
function PopConfig() {
    mv ${launcher_senseauto_config_dir}/resource_cfg/topic_config.yaml.back ${launcher_senseauto_config_dir}/resource_cfg/topic_config.yaml
}
