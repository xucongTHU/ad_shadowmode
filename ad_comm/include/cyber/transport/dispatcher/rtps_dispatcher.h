...
struct Subscriber {
  Subscriber() : reader(nullptr), sub_listener(nullptr) {}
...
class RtpsDispatcher : public Dispatcher {
 public:
  ...
  std::shared_ptr<Subscriber> GetSubscriber(const RoleAttributes& attr) const {
    std::shared_ptr<Subscriber> ret = nullptr;
    uint64_t channel_id = attr.channel_id();
    {
      std::lock_guard<std::mutex> lock(subs_mutex_);
      if (subs_.find(channel_id) != subs_.end()) {
        return subs_.at(channel_id);
      }
    }
    return ret;
  }
 ...
 private:
  ...
  // key: channel_id
  std::unordered_map<uint64_t, std::shared_ptr<Subscriber>> subs_;
