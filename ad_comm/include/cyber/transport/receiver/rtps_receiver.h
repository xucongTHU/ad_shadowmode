...
template <typename M>
bool RtpsReceiver<M>::Matched(uint32_t* matched_count) {
  auto subs = dispatcher_->GetSubscriber(this->attr_);
  if (subs && subs->sub_listener) {
    return subs->sub_listener->Matched(matched_count);
  } else {
    AWARN << "sub listener nullptr";
    return false;
  }
}
