...
bool Client<Request, Response>::ServiceIsReady() const {
  auto rtps_transmiter =
      std::dynamic_pointer_cast<transport::RtpsTransmitter<Request>>(
          request_transmitter_);
  auto rtps_receiver =
      std::dynamic_pointer_cast<transport::RtpsReceiver<Response>>(
          response_receiver_);
  return rtps_transmiter->Matched() && rtps_receiver->Matched();
}
