...
class Node {
 public:
  ...
  friend bool Init(const char*, const PerProcessConfig*);
  ...
  auto CreateService(const std::string& service_name,
                     ...
                     service_callback, const uint64_t& thread_num)
...
auto Node::CreateService(
  ...
  return node_service_impl_->template CreateService<Request, Response>(
      service_name, service_callback, thread_num);
}
