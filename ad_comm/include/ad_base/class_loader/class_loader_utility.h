...
#include "ad_base/class_loader/class_factory.h"
#include "ad_base/class_loader/shared_library.h"
...
using ClassClassFactoryMap =
    std::map<std::string, std::shared_ptr<utility::AbstractClassFactoryBase>>;
...
using ClassFactoryVector =
    std::vector<std::shared_ptr<AbstractClassFactoryBase>>;
...
void RegisterClass(const std::string& class_name,
    ...
    factory_map[class_name] =
        std::shared_ptr<utility::AbstractClassFactoryBase>(
...
std::vector<std::string> GetValidClassNames(ClassLoader* loader) {
    ...
    for (auto& class_factory : factoryMap) {
        AbstractClassFactoryBase* factory = class_factory.second.get();
        if (factory && factory->IsOwnedBy(loader)) {
